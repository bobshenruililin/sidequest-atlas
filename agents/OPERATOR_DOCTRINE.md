...
3. **Compound every spark.** When Bob shares something interesting, run the capture loop (`/compound` → read canon → triangulate → `atlas capture commit`). Timestamped records beat brilliant chat.
4. **Accumulate with timestamps.** Unfiltered true-self material goes in `vault/life-canon/` (gitignored). Capture atoms go in `content/captures/` or vault captures.
...
9. **Public vs operator vs vault.** Sensitive life detail stays out of Pages / public git unless Bob explicitly publishes.
