# General Round — paste-ready packet (24 Aug 2026)

**Do not submit the 23 Aug paste.** Use this file. Kill-list baseline: `docs/general-round-app-2026-08.md`. Debate: `docs/general-round-debate/`.

You: Year 2 BASc Global Health and Development, local, CGPA **4.09 / 4.30** (confirm in SIS). Tung & Ngai already awarded. Laidlaw with Prof. David Bishai ongoing. Martin Scholar, St John’s. Family **stable**. Grey-door **closed**.

This is one form across 40+ internal schemes. Merit readers and need-aware readers see the same answers. “Unrejectable” here means internally consistent and distinctive. It does **not** mean invented hours, lumps, titles, or hardship.

Household figures, rent, salaries, legal names, phones, emails, UID, and HKID stay **as on the form** — not in this memo.

---

## Human jobs before submit (panel cannot do these)

1. Screenshot HKU Portal → Online Applications → Application for Awards → General Round (UG Year 2) → “Important Information.” **That is the deadline.** Do not invent one.
2. Copy Q11 titles, values, and awarding bodies from **letters**.
3. Count certificated unpaid service hours, then pick the Q13 radio band.
4. Read the Tung & Ngai concurrent-award clause. If stacking is forbidden, the pastes already say decline.
5. Confirm CGPA in SIS.

**Sequence:** screenshot the portal first. If General Round is not earlier, send the Youde **local** nomination email first (separate packet — not this form), then submit this form. Reverse if the portal closes sooner. Youde overseas (4 Sep 2026) is a different scheme.

**CV:** attach `cv/shen-ruililin-cv.pdf` to Youde. For General Round, attach the same PDF **only if the portal requests or permits a CV**. Do not invent a second CV.

---

## Kill-list (still law)

1. Q12 “Not applicable.”
2. Q13 “more than 300 hours” with blank or unproven Q14.
3. Q11 “Tung ang Ngai” or HK$500,000 / ~HK$520,000 packaging.
4. Q29 “I cover all educational expenses.”
5. Q30 Laidlaw as unpaid research.
6. Q30 Year-2 “thesis.”
7. Encoding garbage (`It?s`).
8. GEST, tutoring, NEWDAY, Nordic travel, basketball, or Laidlaw as volunteer hours.
9. Invented JCR posts.
10. Need theatre / hiding Tung & Ngai.
11. CS minor declared before SIS.
12. Blank Q33, or Q33 that mentions Youde (especially overseas / 4 Sep).
13. Cloning Tung & Ngai PS scenes (Wudu, seeds, “trusted interface”) into Q29/Q30.
14. Printing GEST calendar dates, LiA cities, or “1 of N” ratios on this form.
15. World Health Summit, MIT Cambridge MA, China CDC as an offer.

---

## Keep as written

| Item | Keep |
| --- | --- |
| Q1 HKSAR PR | Yes (HKID in the merge pack — not in git) |
| Q2 first-generation | No |
| Q3 ancestral home | Shanghai |
| Q5 religion | Christian |
| Q6 school | Po Leung Kuk Choi Kai Yau School (IBDP) 2019–25 |
| Q7–9 SPACE / HKDSE | No / N/A |
| Q10 IB | IBDP 41/45 and the subject list already typed |
| Q15–22 TSFS / NLSFT / UFA / CSSA | No / Not applicable |
| Q23–28 household | **As on form.** Do not rewrite figures into chat or git |
| Q31–32 upload + no-edit | Yes |

---

## Q4 — programme (paste)

```
1. BASc Global Health and Development
2. N/A
```

Not a planned CS minor. SIS first.

---

## Q11 — scholarships (paste after checking letters)

Format the form asked for: year, name, value, awarding body. **Letter wins.** Amounts already listed stay unless a letter contradicts them. Tung & Ngai is the exception: do **not** paste HK$500,000 or a four-year ceiling.

```
2022-23, Po Leung Kuk Choi Kai Yau School Academic Scholarships, HK$6,000, Po Leung Kuk Choi Kai Yau School
2023-24, MIT Hong Kong Innovation Node Youth Fellowship, HK$3,000, MIT Hong Kong Innovation Node
2024-25, Po Leung Kuk Choi Kai Yau School Academic Scholarships, HK$4,000, Po Leung Kuk Choi Kai Yau School
2025-26, [MARTIN LETTER TITLE], HK$1,000, [awarding body from the letter — usually St. John's College, The University of Hong Kong]
2025-26, [OFFICIAL TITLE FROM THE HK$16,000 AWARD EMAIL/LETTER], HK$16,000, [awarding body from the letter]
2025-26, Laidlaw Undergraduate Research and Leadership Programme, HK$12,000, Laidlaw Foundation / HKU Horizons Office
2025-26, Tung and Ngai Foundation Scholarship, [THIS YEAR'S COMMITTED FIGURES FROM THE AWARD LETTER — tuition + living allowance HK$40,000; do not add overseas max HK$80,000 or Endeavour up to HK$100,000 into a lump unless the letter states they have already been disbursed], Tung and Ngai Foundation
2025-26, Tam Wun Tsun HKU Horizons Student Enrichment Award, HK$8,000, HKU / Tam Yik Fong Charitable Foundation
```

If the portal demands one number for Tung & Ngai, use **this year’s committed amount on the letter**. Put contingent pots in words as “as on letter, not yet drawn.” Do not write “1 of 4 HKU scholars” in Q11.

**Tam Wun Tsun letter (31 Mar 2026):** HK$8,000 to support service as Student Co-ordinator on the HKU–Common Purpose Leadership Development Programme, Summer 2026. That is **not** Gannan Horizons. Do not file it as Gannan proof.

Laidlaw award letter matches the listed **HK$12,000**. Copy title and body from that letter.

List everything held. Hiding Tung & Ngai is a different lie.

---

## Q12 — campus / sports (paste)

Replace “Not applicable.” Residence is residence, not an office. Do **not** invent a JCR post, captaincy, or weekly load.

```
St. John's College, HKU — residential member and Martin Scholar, 2025–present. No JCR office held. Time devoted: [HOURS] (term-time residence; do not invent a committee load).

St. John's College Basketball Team — player (not captain), 2025–26. Time devoted: [HOURS].

Laidlaw Undergraduate Research and Leadership Programme — undergraduate research with Prof. David Bishai, HKU School of Public Health, on heat and cardiovascular admissions among older adults (Hospital Authority data), January 2026–present. Funded research, not a student-society office. Time devoted: [HOURS].

Hong Kong Laureate Forum — Forum Ambassador, [YEAR AND MONTHS AS ON THE LETTER]. University activity, not a hall JCR post. Time devoted: [HOURS].
```

Optional **only if still true**:

```
St. John's College Theatre Club — member, 2025–26. Time devoted: [HOURS].
```

Do **not** put GEST, Fukuda, tutoring, NEWDAY, or “unpaid research” here.

---

## Q13 / Q14 — service

**Q13 rule:** Count certificates or timesheets first. Select the **lowest true band** that contains the verified total.

Count only completed, documented, unpaid, non-credit **duty** hours — non-overlapping, inside the form’s eligibility window. Do not count titles, standby, travel, meals, sleep, social time, paid work, academic credit, or participant tourism.

Do **not** select “more than 300 hours” unless the verified eligible sum is **strictly greater than 300**. Default until counted: **lower the band**.

GEST, tutoring, NEWDAY, MEDocGPT, basketball, Laidlaw, and Oslo–Stockholm–Helsinki travel are **not** volunteer hours.

**Q14 paste** (omit line 2 if there is no hour-bearing proof):

```
1. Wu Zhi Qiao (HKU) — Student Coordinator; rural health and infrastructure service, Macha, Gansu; May–Jun 2026; coordinated on-site student service. Verified service hours: [HOURS]. Evidence: [PROOF].

2. HKU Horizons — Gannan fieldwork (official activity name as on the Horizons confirmation); participant; Mar 2026. Verified service hours: [HOURS]. Evidence: [PROOF]. Count only documented service sessions, not travel or meals.

Total verified volunteer service: [TOTAL HOURS] hours.
```

Do not write “1 of 3” on this form. The Student Coordinator **title** is not itself an hour count. If the role was paid, stipend-compensated, or credit-bearing, do not count it unless the form rules explicitly permit that.

Optional **only with proof and counted hours** (do not reconstruct CAS):

```
3. Society of True Light — volunteer helper, Hong Kong International LGBTQ+ Affirmative Practices Conference, 13–14 Jun 2025; hours: [HOURS from duty time only]; proof: certificate of appreciation.
```

---

## Q29 — financial difficulties (paste; ≤200 words)

Family is stable. Tung & Ngai already covers tuition and living. The ask is **contiguous time**, not rent. No hardship story. No “I pay for everything.” No thesis disclaimer. No WZQ scene. No Youde. GEST and London stay out of this box so it does not read as a request to subsidise prestige travel.

**Word count: 160.**

```
My family is financially stable; household details are as declared on this form. I hold the Tung and Ngai Foundation Scholarship, which covers tuition and a living allowance (figures as on the award letter). I will not perform a hardship I do not have.

What I am short of is contiguous time. I tutor part-time for independence and for residual costs existing awards do not reach. Tutoring fragments my week: the hours left between sessions are too short for the work I am at HKU to do. Hong Kong is ageing and heating at the same time, and I am learning how this city can make health evidence trusted, starting with funded research on heat and hospital admissions among older adults.

If a General Round award may be held concurrently with Tung and Ngai, it buys those hours back. If the rules or the award letter forbid concurrent holding, I will decline. The money is settled. The hours are not.
```

---

## Q30 — how you would use it (paste; ≤100 words)

Laidlaw is **funded**. Cash is residual only. Decline if stacking is forbidden. Name the October invited talk as work already on the calendar, not as a completed honour.

**Word count: 74.**

```
This award buys hours, not a second tuition package. If concurrent holding is allowed, I will cut tutoring and return those hours, in contiguous blocks, to my Laidlaw-funded heat-health research with Prof. David Bishai, to coursework, and to preparing an invited talk at GEST Valencia in October 2026. Any cash covers residual research or travel costs Tung and Ngai, Laidlaw, and Horizons do not already pay. If concurrent awards are forbidden, I will decline.
```

Read the Tung & Ngai letter before treating a win as spendable.

---

## Q33 — how you learned of this (paste)

Use only channels actually encountered. Do **not** mention Youde.

```
HKU email from the Academic Advising and Scholarships Office and the AAS “Apply for Scholarships” webpage announcing the General Round Scholarship Application.
```

---

## File checklist (7 slots × 5MB) — merge PDFs; not in git

Do **not** upload the 21-page Tung & Ngai supporting PDF as a General Round file. That pack was a pre-award spray (Laidlaw, Tam, IB, writing/WSC/racing, secondary enrichment, 2020 WZQ emails, projects). General Round only wants documents that back a box on this form.

| Slot | Merge |
| --- | --- |
| 1 | HKID copy (Q1). Not in the T&N pack. |
| 2 | IB Diploma results used for HKU admission (Q10). Pull **one** clean scan from the T&N pack (D3 or F4). Do not upload both. |
| 3 | **Award letters only**, one PDF: CKY academic 2022–23 and 2024–25; MIT Hong Kong Innovation Node; Martin (letter title); the HK$16,000 award under its **official** title; Laidlaw (T&N pack D1); **Tung and Ngai award letter** (not this application pack); Tam Wun Tsun (T&N pack D2). |
| 4 | **Service proofs that appear on Q14**, one PDF: Wu Zhi Qiao **May–Jun 2026** coordinator certificate (the one you have separately — not the 2020 WZQ emails in T&N pack F1); Gannan Horizons **only if** that line is on Q14. Optional: Society of True Light conference volunteer helper, 13–14 Jun 2025 (T&N pack F3) **only if** you add it to Q14 with counted hours. |
| 5–7 | Overflow if a merge exceeds 5MB; SIS CGPA print **only if** the portal asks; CV **only if** the portal asks |

**From the T&N pack, reuse:** D1 Laidlaw letter; D2 Tam Wun Tsun letter; one IB page.

**Have separately — must go into slot 3 or 4:** Tung and Ngai **award** letter (read the concurrent-award clause); Martin letter; MIT Node letter; both CKY academic letters; official HK$16,000 letter; 2026 WZQ certificate; Gannan proof if used on Q14.

**Leave out of General Round** (true, just the wrong form): NEWDAY; GEST invitation as volunteer proof; World Scholar’s Cup; JetRacer; Young Writers trophy photos; Oxford St Anne’s summer-school email; PolyU JRMP; HKUST quantum / HKAGE physics certificates; HKLSS mentorship offer; World Health Summit invitation (visa letter, not attendance, not a form box); STaR enrolment; HipzHub proposal; SPCC teaching-assistant emails; MEDocGPT; 2020 WZQ emails treated as 2026.

Do **not** drop the Tung & Ngai award letter to make the money story easier.

Q31 stays Yes. Q32 stays Yes. You cannot edit after submit.

---

## What this form is not

Not a Youde nomination. Not a second Tung & Ngai personal statement. Not a CEDARS bursary. Not HKU Future Leaders (GHD cannot apply). Not a place to plant LINK, LiA cities, ArtIntel, or a CS minor.

---

## Uncertainty (do not round into facts)

- Year-2 General Round timestamp: portal-only as of 24 Aug 2026 (public apply board last updated **18 Aug 2026**).
- Tung & Ngai concurrent-award clause: unread; letter only.
- Official title of the HK$16,000 award.
- Whether Martin paper says Scholarship, Prize, or Scholar.
- Laidlaw award letter states HK$12,000 for the 2025–26 programme. LiA travel is still a separate line unless that same letter names it.
- Volunteer hour **sum**. Unknown until 2026 WZQ (and optional True Light / Gannan) certificates are counted.
- Tam Wun Tsun funded Common Purpose Summer 2026 coordination, not Gannan.
- Hong Kong Laureate Forum: the T&N pack letter is **2023**. Do not print Sep–Nov 2025 on Q12 unless a 2025 letter exists.
- 2020 WZQ emails in the T&N pack are not proof of May–Jun 2026 coordination.
- 2026/27 local tuition figure: letter / SIS, not invented here.

Accessed 2026-08-24: [AAS apply board](https://aas.hku.hk/apply-scholarships/), [AAS UG schemes](https://aas.hku.hk/for-ugstudies/), [Tung and Ngai AAS card](https://scholar.aas.hku.hk/?action=showonesscheme&ss_id=766), [Foundation benefits](https://tungngaifoundation.org/financial-awards-benefits/), [Laidlaw at HKU Horizons](https://tl.hku.hk/horizons/laidlaw/).
