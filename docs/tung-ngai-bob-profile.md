# Bob — Tung & Ngai read-back profile

**Last updated:** 2026-08-19T21:45Z  
**Use:** Any future agent/session should read this before rewriting interview/complement advice.  
**Plane:** operator docs (no PII). Vault twin may mirror without financials/IDs.

---

## Situation

- **Event:** Tung & Ngai Foundation Selection Panel interview  
- **When:** 2026-07-28, 15:40–15:55 (15 min); window 27–28 Jul  
- **Email brief:** background, achievements, aspirations  
- **Language working assumption:** mostly English; Mandarin strong; Cantonese needs scripts  
- **Status:** past HKU nomination; Foundation finalist

## Academic

- HKU BASc Global Health and Development, Year 1 (2025/26)  
- GPA **4.09/4.30**  
- IB 41 (English A 7; Chinese A 6 + Chinese EE A)  
- Long-term aspiration in submitted materials: CHP cross-boundary disease control; JD **after** UG as regulatory tool only if asked — **not** Year-1 law transfer in interview

## Submitted thesis (do not rewrite; complement)

- Personal Statement: trust / “the seam” / HK as trusted interface / Laidlaw heat-CV / CHP  
- Learning Proposal Q: How can Hong Kong help make China’s public-health capacity trusted and interoperable for the ageing world?  
- Stack in LP: technical core → WZQ / APRU Shenzhen / LiA makesense Americas → T&N community → forum interoperability proposal

## Timeline truth (operator-corrected 2026-07-22)

| When | What | Tense |
|------|------|-------|
| High school | Sustained research track | Done |
| Jan 2026→ | Laidlaw heat × cardiovascular admissions (Bishai / HA data) | Ongoing |
| Mar 2026 | Gannan Horizons fieldwork (Wudu, temple-medical, three bags of seeds) | Done |
| May–Jun 2026 | Wu Zhi Qiao (Mainland rural health/infrastructure; coordinator track) | **Done** |
| Oct 2026 | Laidlaw conference, **London** | Upcoming |
| Oct 2026 | **GEST** environmental science conference, **Valencia** — **invited speaker** | Upcoming |
| Winter 2026/27 | **LINK-S** Shanghai ([tl.hku.hk/link/about](https://tl.hku.hk/link/about/)) | Upcoming |
| Next summer | Laidlaw LiA with **makesense** — Medellín, Colombia / Mexico City, Mexico | Upcoming / planned |
| Degree aim | **China CDC** field placement (GHD) | Aiming |

## Binding interview constraints (Bob)

- Soft identity: **really careful**. Do not lead with Shanghai-born / HK-raised controversy. Align with Foundation: affection for Hong Kong + commitment to China/nation + national cultural confidence — enacted through **work**, not birthplace drama.  
- Research-heavy is OK if framed as prerequisite for service (cohort T&N scholars are often research-forward).  
- Bank WZQ (done) + LiA (upcoming) + China CDC aim.  
- Steal cohort structures in detail; do not clone biographies.  
- Complements to PS/LP only — do not rewrite submitted texts.

## Foundation alignment keywords (from mission + PPT)

- profound affection for Hong Kong  
- contribute to society and the nation  
- social responsibility + national cultural confidence  
- lifelong commitment to local Hong Kong community and China  
- independent learning, cross-cultural communication, leadership  
- spirit of endeavour / innovation / entrepreneurship (GBA)  
- compulsory Mainland exchange; Endeavour Fund GBA/China projects  
- HKSAR policy areas include **Medical Services** and Environmental Protection / Sustainable Development (PPT)

## Competitive stance (see complements doc)

Top-quartile among researched finalist/scholar sample for HKU lane; not guaranteed. Edge = research continuity + completed Mainland rural service + invited international speaking + China CDC aim + Global South LiA return path.

## Related files

- `docs/hkuww-exchange-primer.md` — 2027/28 HKUWW primer (two equal stacks; tuition wedges; Oct 2025 full list vs March leftovers).
- `docs/winter-2026-27-primer.md` — winter 2026/27 LINK-S vs IAO Recognised list (verdicts; NEWDAY DNA; access 22 Aug 2026).
- `docs/winter-2026-27-fact-sheets.md` — one-page winter dossiers (fork, live options, closed files, Chile craving vs product).
- `docs/exchange-review-ghd-hkuww.md` — adversarial review (2026-08-19). Keep capstone/GPA/LSE-Health-Policy/UBC findings; the "Chicago N/A" line was a leftover-list artefact — see primer editor's note.  
- `docs/tung-ngai-interview-prep.md`  
- `docs/tung-ngai-english-interview-scripts.md` (+ CMN/YUE)  
- `docs/tung-ngai-scholar-cohort-dataset.md`  
- `docs/tung-ngai-complements-and-competitiveness.md`  
- `content/captures/2026-07-22-tung-ngai-interview.yaml`
